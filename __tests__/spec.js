...
describe("the game", function () {
    ...
    function expectCountdownToStart() {
        startGame();
        let clock = getCountdown();
        expect(Number(clock.innerHTML)).toBeGreaterThanOrEqual(0);
    }

    it('resets the counter when answer a question', function () {
        // expectCountdownToStart();
        startGame();

        goToNextQuestion();

        let clock = getCountdown();
        expect(Number(clock.innerHTML)).toBeLessThanOrEqual(9);
    });
});